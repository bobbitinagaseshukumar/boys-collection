...
export default function ShopPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-24 pb-12 min-h-screen">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-8">
          ...
          <div className={`${showFilters ? 'flex' : 'hidden'} md:flex flex-wrap items-center gap-3 w-full md:w-auto`}>
            ...
            <button
              ...
              className={`px-4 py-2 text-xs rounded-full border transition-all min-h-[36px] ${
                !selectedCategory ? 'bg-[#d4af37] text-[#0a0a0f] border-[#d4af37]' : 'bg-white/[0.04] text-white/50 border-white/10 hover:border-white/20'
              }`}
            >
              All
            </button>
            {categoriesList.map((cat) => (
              <button
                ...
                className={`px-4 py-2 text-xs rounded-full border transition-all min-h-[36px] ${
                  selectedCategory === cat.slug ? 'bg-[#d4af37] text-[#0a0a0f] border-[#d4af37]' : 'bg-white/[0.04] text-white/50 border-white/10 hover:border-white/20'
                }`}
                data-cursor="hover"
              >
                {cat.name}
